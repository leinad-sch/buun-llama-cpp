...
int cli_context::run() {
    ...
    while (true) {
        ...
        // skip empty messages
        if (buffer.empty()) {
            // fork fix (feedback_llama_cli_repl_eof_loop): on stdin EOF an empty
            // read repeats forever ("> " spin). A scripted / non-interactive run
            // (e.g. `llama-cli -p ... < /dev/null`) must terminate instead of loop.
            if (feof(stdin)) {
                break;
            }
            continue;
